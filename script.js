...
let books = [
  {
    ad: "Crime and Punishment",
    yazar: "Dostoyevski",
    adet: 120.99,
    tane: 0,
    href: "images/crime.jpg",
  },
  ...
  },
  {
    ...
    adet: 22.22,
    tane: 0,
    href: "images/hayvan-ciftligi.jpg",
  },
  {
  ...
  },
  {
    ...
    adet: 22.22,
    tane: 0,
    href: "images/beyaz-dis.jpg",
  },
];
var sum = 0;
let eklenenkitaplar = [];
function saveToLocalStorage() {
  localStorage.setItem("cartItems", JSON.stringify(eklenenkitaplar));
}
...
document.getElementById("searchInput").addEventListener("keyup", function () {
  ...
  cards = document
    ...
    .getElementsByClassName("bookCard");
...
function performLogin() {
  ...
  if (
    ...
    alert(
      "Giriş başarısız. Lütfen kullanıcı adı, e-posta ve şifrenizi kontrol edin."
    );
  }
}
...
let ab = 1;
function createBookElement(book, imagePath, deger) {
  if (deger == true) {
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (storedItems) {
    eklenenkitaplar = JSON.parse(storedItems);

    let uniqueBooks = [];

    eklenenkitaplar.forEach(function (book) {
      // Aynı id'ye sahip kitapları kontrol et
      let isBookAlreadyAdded = uniqueBooks.some(function (existingBook) {
        return existingBook.ad === book.ad;
      });

      // Eğer aynı id'ye sahip kitap yoksa, yeni diziye ekle
      if (!isBookAlreadyAdded) {
        uniqueBooks.push(book);
      }
    });

    // Şimdi uniqueBooks dizisi, aynı id'ye sahip kitapları içermeyecek
    console.log(uniqueBooks);

    // Display the items in the cart
    uniqueBooks.forEach(function (book) {
      let bookElement = createBookElement(book, book.href, false);
      document.getElementById("sepet").appendChild(bookElement);
      console.log(bookElement.id);
    });
    let sum = 0;
    // Update the total count and price
    uniqueBooks.forEach(function (eleman) {
      sum += eleman.adet * eleman.tane;
      sum = parseFloat(sum.toFixed(4));
    });

    sayı.innerText = eklenenkitaplar.length;
    document.getElementById("fiyat").innerText = "Fiyat:" + sum;
  }
});
